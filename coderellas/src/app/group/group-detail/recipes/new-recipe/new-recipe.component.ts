...
//BEFORE USING A DELIMITER CONCEPT
...
export class NewRecipeComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.recipeForm = this.fb.group({
      ...
      instructions: ['', Validators.required],  // This will hold the final instructions with delimiters
      cookingTime: ['', [Validators.required, Validators.min(1)]],
      difficultyLevel: ['Easy', Validators.required],
      recipeType: ['Vegetarian', Validators.required],
      public: [true],
  ...
  addInstructionStep(): void {
    ...
    this.currentStep = ''; // Clear the input field for the next step

    // Join all steps into a single string with a delimiter and set it to the form control
    const instructionsWithDelimiter = this.instructionSteps.join('|||'); // Using '|||' as a delimiter
    ...
    this.http.post(`http://127.0.0.1:5000/add-recipe/${this.groupId}`, formData).subscribe(
      {
        next: (response) => {
          ...
          this.instructionSteps = []; // Clear the steps after submission
          this.router.navigate(['/category-detail', this.groupId]);
        },
        error: (error) => {
          console.error('Error adding recipe:', error);
